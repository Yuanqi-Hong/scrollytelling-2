...
function ready ([world, railway]) {
  ...
  svg
    ...
    .attr('class', 'train')
    .attr('id', d => {
      let cleanedName = d.properties.name.replace(/ /g, '-')
      return cleanedName
    })
    .attr('d', path)
    .attr('fill', 'none')
  ...
  let trainLine = d3.select('#China-Europe-Block-Train')
  ...
  function render () {
    ...
    // update scales
    // note it's newWidth and newHeight now
    // except this will break our map
    // projection.fitSize([newWidth, newHeight], trains)

    // reposition your elements
